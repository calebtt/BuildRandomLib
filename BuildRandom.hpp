...
namespace BuildRandom
{
	...
	[[nodiscard]] constexpr static auto BuildRandomVector(const CountType maxLength, const CountType minLength = 3) noexcept
	{
		...
		std::vector<T> currentBuiltString(tLength, T{});
		std::generate(std::begin(currentBuiltString), std::end(currentBuiltString), getRandomElement);
		return currentBuiltString;
	}
	/// <summary>
	///	Returns a vector of <c>std::string</c> with randomized content using a uniform distribution.
	///	<para>NOTE:
	...
	///	</para>
	///	<para><a href="https://docs.microsoft.com/en-us/cpp/cpp/fundamental-types-cpp/">MSDOCS</a></para>
	/// </summary>
	...
	/// <summary>
	///	Returns a vector of <c>std::wstring</c> with randomized content using a uniform distribution.
	/// </summary>
	///	<param name="numberOfStrings">the number of strings in the returned vector.</param>
	///	<param name="maxLength">the maximum length of the strings in the returned vector.</param>
	///	<param name="minLength">the minimum length of the strings in the returned vector.</param>
